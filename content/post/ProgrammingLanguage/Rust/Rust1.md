...
fn main() {
    ...
    // 旧写法：占位符和绑定名分开，绑定名跟在格式字符串后面
    println!("姓名：{}，成绩：{}", name, score);

    // 新写法（Rust 1.58 起）：直接把绑定名写进占位符里，更直观
...
> **本教程约定**：从这里起，所有代码示例统一使用 `{绑定名}` 的新写法。这是现代 Rust 代码里的推荐风格，代码更简洁，绑定和占位符的对应关系一目了然。
...
Rust 用 `let` 创建绑定。绑定创建时必须立刻初始化（赋值），但不一定需要写出类型——编译器会根据赋的值推断：
...
Rust 中 `let` 默认创建**不可变绑定**（immutable binding）。绑定建立后再次赋值，编译器会报错：
...
需要**可变绑定**时，加 `mut` 关键字，此时才真正称得上"变量"：
...
这个设计是刻意的。在实际项目中，大量绑定是"赋值一次，之后只读"的，默认不可变能避免不少因为意外修改导致的 bug。需要修改时再加 `mut`，代码的意图也更清晰。

### 绑定遮蔽

Rust 允许在同一作用域内用 `let` 重新声明同名绑定，后者会遮蔽（shadow）前者：
...
遮蔽和 `let mut` 的区别：`let mut` 改变的是同一个绑定的值，类型不能变；遮蔽是创建了一个全新的绑定，类型可以完全不同。
