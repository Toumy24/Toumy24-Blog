...
用**解构（destructuring）**把元组的各字段分别绑定到新绑定，比逐个用 `.0` `.1` 访问更清晰：
...
```text
// 方式一：列举所有初始值
let 绑定名: [元素类型; 长度] = [值1, 值2, ..., 值N];

// 方式二：重复值语法（所有元素用同一个值初始化）
let 绑定名: [元素类型; 长度] = [初始值; 长度];
...
`insert` 会转移键和值的所有权。如果键或值是 `String`，插入后原绑定不可用：
